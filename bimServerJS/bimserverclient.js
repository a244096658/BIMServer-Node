...
//remove those part. 
	...
	othis.webSocket=new BimServerApiWebSocket.BimServerApiWebSocket(baseUrl, othis);//new add 04.05.2017
...
//    		othis.notifier.resetStatusQuick();
	...
	this.checkin = function(topicId, project, comment, file, deserializerOid, progressListener, success, error){
		...
		var formData = require("form-data");//for nodejs server use purpose.
	...
	this.call = function(interfaceName, methodName, data, callback, errorCallback, showBusy, showDone, showError) {
		...
		return othis.multiCall([[
			...
			if (data.exception == null) {
				if (callback != null) {
					callback(data.result);//data.result pass to callback(data). so only result is log in the console. But we can modify it based on our requirment.
				}
			} else {
				if (errorCallback != null) {
